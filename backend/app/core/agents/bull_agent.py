...
class BullAgent:
    ...
    def __init__(self):
        """Initialize the bull agent with personality and LLM."""
        ...
        logger.debug("BullAgent initialized")
    ...
    def _build_sources(
        ...
        logger.info(f"Bull analysis: Total news items: {len(news_items)}, Bullish filtered: {len(bullish_news)}")
        ...
        logger.debug(f"Bull analysis: Added {count} news sources")
        return sources
    ...
    async def analyze(
        ...
        # Execute task asynchronously (this is the key improvement)
    ...
    def _parse_result(self, result: str, sources: list[Source]) -> AgentAnalysis:
        ...
        try:
            # Use SimpleAgent's built-in JSON parser
            ...
            logger.error(f"Failed to parse bull analysis response: {str(e)}")
            # Fallback parsing
            return AgentAnalysis(
                agent_type="bull",
