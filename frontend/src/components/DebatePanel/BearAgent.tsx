...
const getImportanceLevel = (confidence: number): { label: string; stars: number; color: string } => {
  if (confidence >= 0.8) return { label: 'Critical', stars: 3, color: '#ff4757' };
  ...
  return (
    <div
      className={`stock-card p-6 transition-all duration-300 h-full ${
        isActive
          ? 'border-stock-danger/50 shadow-lg'
          : analysis
          ? 'border-stock-danger/30'
          : ''
      }`}
      style={{
        background: analysis || isActive
          ? 'linear-gradient(135deg, rgba(239, 83, 80, 0.08) 0%, rgba(30, 34, 45, 1) 100%)'
          : undefined,
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold text-stock-danger">Bearish Analysis</h3>
          <p className="text-xs text-stock-text-secondary font-medium">
            {roundNumber && roundNumber > 1 ? `Round ${roundNumber}` : 'Risk Assessment'}
          </p>
        </div>
        {analysis && (
          <div className="relative group">
            <div className="bg-gradient-to-br from-stock-danger/20 to-stock-danger/5 px-6 py-4 rounded-lg border border-stock-danger/30">
              <div className="flex items-center gap-3">
                <div className="text-2xl font-black text-stock-danger">
            ...
            <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20">
              <div className="bg-stock-danger/90 backdrop-blur-sm px-3 py-1 rounded-md">
      ...
      {isActive && !analysis && (
        <div className="py-12 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            <div className="loading-dot bg-stock-danger" />
            <div className="loading-dot bg-stock-danger" />
            <div className="loading-dot bg-stock-danger" />
          </div>
          <p className="text-stock-danger text-base font-semibold">Analyzing bearish factors...</p>
          <p className="text-sm text-stock-text-muted mt-2">Identifying potential risks</p>
        </div>
      )}
        ...
        <div className="flex flex-col">
          ...
          {analysis.arguments.length > 0 && (
            <div className="space-y-3 mt-4">
              <p className="text-sm text-stock-danger font-semibold">
                Key Concerns
              </p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-6 h-6 bg-stock-danger/20 rounded flex items-center justify-center flex-shrink-0 text-xs font-semibold text-stock-danger">
                          {idx + 1}
                        </div>
          ...
          {analysis.sources && analysis.sources.length > 0 && (
            <div className="mt-4 pt-4 border-t border-stock-danger/20">
              <p className="text-xs text-stock-text-secondary font-semibold mb-2">Data Sources</p>
              <div className="flex flex-wrap gap-2">
                {analysis.sources.map((source, idx) => (
                  <div
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-stock-danger/10 border border-stock-danger/20 rounded text-[10px]"
                  >
                    <span className="text-stock-danger font-semibold">{source.type}</span>
                    {source.url ? (
                      <a
                        href={source.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-stock-text-secondary hover:text-stock-danger transition-colors"
                        title={source.name}
                      >
